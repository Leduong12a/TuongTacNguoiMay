...
export const SearchFriend: React.FC = () => {
  ...
  const [inviteMessage, setInviteMessage] = useState(
    "Xin chào, mình là Phùng Văn Duy. Kết bạn với mình nhé!",
  );

  // Alert Modals state
  const [alertType, setAlertType] = useState<
    "sent" | "unfriended" | "blocked" | "unblocked" | "accepted" | null
  >(null);
  ...
  const handleCancelInvite = (user: UserItem) => {
    setUsers((prev) =>
      prev.map((u) => (u.id === user.id ? { ...u, status: "none" } : u)),
    );
  ...
  const handleAcceptInvite = (user: UserItem) => {
    setUsers((prev) =>
      prev.map((u) => (u.id === user.id ? { ...u, status: "friends" } : u)),
    );
    ...
    <div className="w-full h-full flex flex-col bg-[#F7F9FC] p-6 md:p-8 overflow-y-auto">
      ...
      <div className="max-w-[720px] mx-auto w-full mb-8">
        ...
        <p className="text-[14px] text-slate-500 mt-1.5">
          Nhập số điện thoại hoặc tên để tìm kiếm và kết nối với những người
          xung quanh.
        </p>
      </div>
        ...
        {filteredUsers.length > 0 ? (
            ...
            {filteredUsers.map((user) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`w-14 h-14 rounded-xl ${user.avatarBg} text-white font-bold text-[18px] flex items-center justify-center relative shadow-sm transition-transform duration-300 group-hover:scale-105`}
                  >
                    {user.avatar}
                    <span
                      className={`absolute bottom-[-1.5px] right-[-1.5px] block h-3 w-3 rounded-full ring-[2px] ring-white ${user.status === "blocked" ? "bg-slate-400" : "bg-green-500"}`}
                    />
                  </div>
          ...
          <div className="flex-1 bg-white border border-dashed border-slate-250 rounded-2xl p-12 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <p className="text-[13.5px] text-slate-400 mt-1 max-w-[320px]">
              Không tìm thấy người dùng nào khớp với số điện thoại hoặc tên "
              {searchQuery}".
            </p>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl p-7 max-w-[420px] w-full mx-4 shadow-2xl border border-slate-100/80 z-50 transform transition-all duration-300 scale-100 flex flex-col">
            ...
            <div className="bg-slate-50/70 border border-slate-100 rounded-xl p-4 flex items-center gap-3.5 mb-5">
              <div
                className={`w-12 h-12 rounded-xl ${selectedUser.avatarBg} text-white font-bold text-[16px] flex items-center justify-center shadow-inner`}
              >
                {selectedUser.avatar}
              </div>
              <div className="flex flex-col">
                ...
                <span className="text-[12px] text-slate-400 font-semibold mt-0.5">
      ...
      {alertType && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl p-8 max-w-[360px] w-full mx-4 shadow-2xl border border-slate-100 z-50 transform transition-all duration-300 scale-100 flex flex-col items-center text-center">
            ...
            {alertType === "sent" && (
              <>
                <h4 className="text-[18px] font-bold text-slate-800 mb-2">
                  Đã gửi lời mời thành công!
                </h4>
                <p className="text-[13.5px] text-slate-500 mb-6 leading-relaxed">
                  Lời mời kết bạn đã được gửi đến{" "}
                ...
                <div className="flex flex-col gap-2 w-full">
                  ...
                  <button
                    onClick={() => {
                      setAlertType(null);
                      alert(
                        `Đang chuyển tới cuộc trò chuyện với ${alertTargetName}...`,
                      );
                    }}
            ...
            {alertType === "unfriended" && (
              <>
                ...
                <p className="text-[13.5px] text-slate-500 mb-6 leading-relaxed">
                  ...
                  không còn là bạn bè trên ChatApp.
                </p>
                <button
                  onClick={() => setAlertType(null)}
                  className="w-full h-11 bg-[#0056C6] hover:bg-[#0047A5] text-white text-[14px] font-bold rounded-xl transition-all cursor-pointer focus:outline-none"
            ...
            {alertType === "blocked" && (
              <>
                ...
                <p className="text-[13.5px] text-slate-500 mb-6 leading-relaxed">
                  ...
                  vào danh sách đen chặn liên lạc.
                </p>
                <button
                  onClick={() => setAlertType(null)}
                  className="w-full h-11 bg-[#0056C6] hover:bg-[#0047A5] text-white text-[14px] font-bold rounded-xl transition-all cursor-pointer focus:outline-none"
            ...
            {alertType === "accepted" && (
              <>
                ...
                <p className="text-[13.5px] text-slate-500 mb-6 leading-relaxed">
                  ...
                  đã trở thành bạn bè trên ChatApp.
                </p>
                <div className="flex flex-col gap-2 w-full">
                  <button
                    onClick={() => {
                      setAlertType(null);
                      alert(
                        `Đang mở hộp thoại trò chuyện với ${alertTargetName}...`,
                      );
                    }}
                    className="w-full h-11 bg-[#0056C6] hover:bg-[#0047A5] text-white text-[14px] font-bold rounded-xl transition-all cursor-pointer focus:outline-none"
                  >
